...
public class Menu_Register_Page3 extends AppCompatActivity {
    ...
    private void verifyP3Inputs() {
        ...
        if(!Static_InputCheck.checkLength(id, 3)){
        ...
        if(!Static_InputCheck.checkLength(pass, 3)){
        ...
        if(!Static_InputCheck.checkLength(confirmPass, 3)){
        ...
        if(pass.equals(confirmPass)){
            new Thread(new NetworkCommunication(Static_HashCompiler.compileRegisterChoice("page3", id, null))).start();
        }else{
            Message message = handler.obtainMessage();
            Bundle bundle = new Bundle();
    ...
    private void checkP3Ready(){
        ...
        if(registerP3ready){
            ...
            new Thread(new NetworkCommunication(Static_HashCompiler.compileRegisterChoice("finishRegister", null, userInfo.getAccount()))).start();
    ...
    private class NetworkCommunication implements Runnable {
        ...
        private void page3() {
            ...
            HashMap communication = Static_HashCompiler.compileCheckID((String) info.get("info"));
        ...
        private void finishRegister() {
            ...
            HashMap communication = Static_HashCompiler.compileFinishRegister((User_Account) info.get("acc"));
