...
Procedimento gestão de nomes a atribuir aos artefactos gerados no ambito do processo. Por ex. nomear um relatório de tarefa/esforço, um processo ou uma ata. Este processo também é responsável por garantir a enumeração das revisões seguindo um padrão.
...
* Nomeação de artefactos utilizando as seguintes regras:
  ...
  * Restantes artefactos:
    * Os nomes dos restantes artefactos que aqui não se encontram descritos, deverão ter sempre no inicio a unidade e a tipologia a que se refere o documento, como por exemplo, QUA_REL_ACOMPANHAMENTO_GES, GES_REL_RISCOS.
      * Deve-se tentar sempre manter o nome curto, mas que seja perceptivel o conteudo do documento.
